# reposter.py (senza AIORateLimiter)
import logging
import os
...
SOURCE_CHANNEL = os.getenv("SOURCE_CHANNEL", "-1001114235602")  # @username oppure -100...
TARGET_CHANNELS: List[str] = os.getenv("TARGET_CHANNELS", "@UmoreSottileOfferte,@ilregnodellacina,@loscrignodelleofferte,-1001165780142,@lareginadelleofferte,@ReginaDelleOfferte,-1001223224860,-1001084518839,-1003105040660").split(",")
...
async def repost_post_to_targets(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not msg or not msg.chat or not is_source_chat(msg.chat, SOURCE_CHANNEL):
        return
    ...
    for target in (t.strip() for t in TARGET_CHANNELS if t.strip()):
        try:
            if REPOST_MODE == "forward":
                ...
                if msg.text:
                    ...
                    await msg.copy(chat_id=target, caption=caption, parse_mode=ParseMode.HTML)
                else:
                    await msg.copy(chat_id=target)
            log.info("Ripubblicato su %s", target)
